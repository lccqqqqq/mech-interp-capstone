...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        )  # each attention head attents to n_embd / n_head elements. We get this by multipling the n_embd matrix by a Q,K,V matrix of size n_embd, n_embd / n_head.
        ...
        )  # this is the Q, K, V matrix, in a batch
        ...
        assert (
            config.n_embd % config.n_head == 0
        ), f"Embedding dimensions {config.n_embd} must be divisible by the number of heads {config.n_head}"
    ...
    def forward(self, x):
        ...
        B, T, C = (
            x.size()
        )  # B is batch size, T is sequence length, C is embedding dimensions
        assert (
            C == self.n_embd
        ), f"Input embedding dimensions {C} does not match the model embedding dimensions {self.n_embd}"
        # B sentences, each sentence has T words, each word is represented in C dimensions
        x_proj = self.c_attn(
            x
        )  # This will make up the Q, K, V matrix. Sends x to 3x dimensions for each matri
        q, k, v = (
            x_proj[:, :, :C],
            x_proj[:, :, C : 2 * C],
            x_proj[:, :, 2 * C : 3 * C],
        )  # split the 3x dimensions into Q, K, V. all same size as x
        ...
        )  # (B, nh, T, T). doing sig(q_i*k_j)/sqrt(len(k)) to find s_ij (NLP notes)

        mask = torch.ones(T, T, device=x.device) * float("-inf")
        mask = torch.triu(
            mask, diagonal=1
        )  # can't look ahead. in first word can only look at first word. not ahead
        ...
        y = (
            att @ v
        )  # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs). z_i = sum a_ij v_j
        y = (
            y.transpose(1, 2).contiguous().view(B, T, C)
        )  # concatenate the n_head attention heads
        y = self.c_proj(y)
...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        )  # feed forward layer. just fancy shmancy syntax, very basic tho
...
class Transformer(nn.Module):
    def __init__(self, config):
        ...
        self.positional_embeddings = nn.Parameter(
            torch.zeros(self.max_seq_length, self.n_embed)
        )
...
if __name__ == "__main__":
    ...
    summary(model, input_size=(5000, 65, 2))
