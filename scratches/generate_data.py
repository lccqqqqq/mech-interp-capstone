...
def generate_springdata(num_samples=1000, sequence_length=10, plot=False):
    ...
    train_omegas = (
        torch.rand(num_samples) * delta_omega / 2 + omegas_range[0] + delta_omega / 4
    )
    # middle half of the omega interval is the training set
    train_deltat = (
        torch.rand(num_samples) * 2 * torch.pi / (train_omegas)
    )  # cos(wt) has period 2pi/w. so deltat>2pi/w is redundant

    start = 0
    skip = 1
    train_times = (
        torch.arange(start, start + skip * sequence_length + 1, step=skip)
        .unsqueeze(0)
    ...
    x_train = x0_train * torch.cos(train_omegas_unsq * train_times) + (
        v0_train / train_omegas_unsq
    ) * torch.sin(train_omegas_unsq * train_times)
    v_train = -x0_train * train_omegas_unsq * torch.sin(
        train_omegas_unsq * train_times
    ) + v0_train * torch.cos(train_omegas_unsq * train_times)
    # stack x and v
    ...
    test_omegas_high = (
        torch.rand(num_samples // 4) * delta_omega / 4
        + omegas_range[1]
        - delta_omega / 4
    )
    ...
    x_test = x0_test * torch.cos(test_omegas_unsq * test_times) + (
        v0_test / test_omegas_unsq
    ) * torch.sin(test_omegas_unsq * test_times)
    v_test = -x0_test * test_omegas_unsq * torch.sin(
        test_omegas_unsq * test_times
    ) + v0_test * torch.cos(test_omegas_unsq * test_times)
    # stack x and v
    sequences_test = torch.stack(
        (x_test, v_test), dim=2
    )  # Shape: (num_samples, sequence_length, 2)
    ...
    if plot:
        ...
        plt.title(
            f"{train_omegas.shape[0]} Training and {test_omegas.shape[0]} Test Omegas"
        )
        plt.legend()
        plt.show()
...
def omega1to2(w1, w2, num_samples=1000, sequence_length=10):
    ...
    train_times = (
        torch.arange(1, sequence_length + 2).unsqueeze(0).repeat(num_samples, 1)
    )
    train_deltat = (
        torch.rand(num_samples) * 2 * torch.pi / (w1s)
    )  # cos(wt) has period 2pi/w. so deltat>2pi/w is redundant
    train_times = train_times * train_deltat.unsqueeze(1)

    x_train_w1 = torch.cos(w1s.unsqueeze(1) * train_times)
    v_train_w1 = -w1s.unsqueeze(1) * torch.sin(w1s.unsqueeze(1) * train_times)
    sequences_w1 = torch.stack(
        (x_train_w1, v_train_w1), dim=2
    )  # Shape: (num_samples, sequence_length, 2)
    ...
    sequences_w2 = torch.stack(
        (x_train_w2, v_train_w2), dim=2
    )  # Shape: (num_samples, sequence_length, 2)
...
def generate_dampedspringdata(
    num_samples=1000, sequence_length=10, plot=False, deltat_mult=1
):
    # Generate x = cos(wt), v = -w*sin(wt). trains on omegas between 0.5pi and 1pi, tests on 0.25pi-0.5pi and 1pi-1.25pi
    omegas_range = [0.25 * torch.pi, 1.25 * torch.pi]
    delta_omega = omegas_range[1] - omegas_range[0]
    ...
    test_deltat = (
        torch.rand(test_omegas.shape[-1])
        * 2
        * torch.pi
        / (sequence_length * test_omegas)
    )
    ...
    def plot_random(sequences, gammas, omegas, times):
        ...
        plt.title(
            f"Sequence {random_index} Plotted Till Time {time[-1]:.3f}s\ngamma = {gammas[random_index]:.2f}, omega = {omegas[random_index]:.2f}, x0 = {x[0]:.2f}, v0 = {v[0]:.2f}"
        )
        plt.legend()
        plt.show()
    ...
    def gen_underdamped(omegas_0, gammasinp, train_times):
        ...
        gw = gammas / omegas
        x0, v0 = get_random_start(train_times.shape), get_random_start(
            train_times.shape
        )
    ...
    sequences_train_under = gen_underdamped(
        omegas_train_under, gammas_train_under, times_train_under
    )
    ...
    sequences_test_under = gen_underdamped(
        omegas_test_under, gammas_test_under, times_test_under
    )

    if plot:
        plot_random(
            sequences_train_under,
            gammas_train_under,
            omegas_train_under,
            times_train_under,
        )
    ...
    def gen_overdamped(omegas_0, gammasinp, train_times):
        ...
        v = exp_prec * (
            A * (omegas - gammas) * torch.exp(wt)
            - B * (omegas + gammas) * torch.exp(-wt)
        )
        seq = torch.stack((x, v), dim=2)
        return seq
    ...
    gammas_train_over = (
        torch.rand(omegas_train_over.shape[0]) * (omega_max - omegas_train_over)
        + omegas_train_over
    )
    times_train_over = train_times[num_under_train:]
    sequences_train_over = gen_overdamped(
        omegas_train_over, gammas_train_over, times_train_over
    )

    omegas_test_over = test_omegas[num_under_test:]
    omega_max = omegas_test_over.max() + delta_omega / 4
    gammas_test_over = (
        torch.rand(omegas_test_over.shape[0]) * (omega_max - omegas_test_over)
        + omegas_test_over
    )
    times_test_over = test_times[num_under_test:]
    sequences_test_over = gen_overdamped(
        omegas_test_over, gammas_test_over, times_test_over
    )

    # if plot:
    #     plot_random(sequences_train_over, gammas_train_over, omegas_train_over, times_train_over)

    sequences_train_damped = torch.cat(
        (sequences_train_under, sequences_train_over), dim=0
    )
    indices = torch.randperm(sequences_train_damped.shape[0])[
        : len(sequences_train_under)
    ]
    ...
    sequences_test_damped = torch.cat(
        (sequences_test_under, sequences_test_over), dim=0
    )
    indices = torch.randperm(sequences_test_damped.shape[0])[
        : len(sequences_test_under)
    ]
    ...
    os.makedirs("spring_data_damped", exist_ok=True)
    torch.save(
        {
        ...
        f"spring_data_damped/damped{deltat_mult}_data.pth",
    )
    # if plot:
    #     plt.hist(train_omegas, color = 'b', label = 'Training Omegas', bins=20)
    #     plt.hist(test_omegas, color = 'r', label = 'Test Omegas', bins=20)
...
def generate_linregdata(num_samples=5000, sequence_length=65):
    ...
    x_test = torch.empty(num_test, sequence_length).uniform_(
        -1, 1
    )  # Fill with values between -1 and 1 initially
    mask = x_test < 0  # Create a mask for negative values
    x_test[mask] = (
        x_test[mask] * (thigh - tlow) - tlow
    )  # Adjust negative values to be between -1 and -0.75
    x_test[~mask] = (
        x_test[~mask] * (thigh - tlow) + tlow
    )  # Adjust positive values to be between 0.75 and 1
    ...
    w_train = w_test = torch.empty(
        num_samples,
    ).uniform_(-1, 1)
    y_train = w_train.unsqueeze(-1) * x_train

    x_train_exp = x_train.unsqueeze(
        2
    )  # Shape becomes (num_samples, sequence_length, 1)
...
def plot_dampedspringdata(mult=1):
    ...
    for i, plottype in enumerate(["underdamped", "overdamped"]):
        ...
        color = colors[i]
        sequence, omega, times, gamma, deltat = (
            sequences_train[index],
            omegas_train[index],
            times_train[index],
            gammas_train[index],
            deltats_train[index],
        ...
        axs[0].plot(
            ks, sequence[:, 0], label=rf"{plottype} $x_k$" + append, color=color
        )
        axs[1].plot(
            ks,
            sequence[:, 1],
            label=rf"{plottype} $v_k$" + append,
            color=color,
            linestyle="--",
        )
...
if __name__ == "__main__":
    generate_dampedspringdata(num_samples=5000, sequence_length=65, plot=True)
    # plot_training_data()
    # playground()
